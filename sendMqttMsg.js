...
if (process.env.MY_MQTT_KEY)MY_MQTT_KEY = process.env.MY_MQTT_KEY;
if (process.env.MY_MQTT_ADDRESS)MY_MQTT_ADDRESS = process.env.MY_MQTT_ADDRESS;
if (process.env.MY_MQTT_PORT)MY_MQTT_PORT = process.env.MY_MQTT_PORT;
...
async function sendMqttMessage(msgTopic,text){
    ...
    return new Promise(resolve =>{
        mqttClient = mqtt.connect(`mqtt://${MY_MQTT_ADDRESS}:${MY_MQTT_PORT}`,options);
        mqttClient.on(`connect`,function(){
            mqttClient.subscribe(`${msgTopic}`,function(err){
                if(!err){
                    mqttClient.publish(`${msgTopic}`, text)
                }
            });
        })
        mqttClient.on(`message`,function(topic,payload,packet){
