...
!(async() => {
	...
	for (i = 0; i < cookiesArr.length; i++) {
		if (cookiesArr[i]) {
			...
			TempBaipiao = "";
			strGuoqi="";
            myBeanInfo = new Object();
            myBeanInfo.index = $.index;
            ...
            console.log(JSON.stringify(myBeanInfo));
			mqttMsg.sendMqttMessage(`${MY_MQTT_BEANFULLINFO_TOPIC}/${myBeanInfo.index}`,JSON.stringify(myBeanInfo));
...
async function showMsg() {
	...
	if ($.levelName || $.JingXiang){
		...
		if ($.levelName) {
            ...
            myBeanInfo.levelName = $.levelName;
		}
	...
	if (llShowMonth) {
        ...
        myBeanInfo.allincomeBean = $.allincomeBean;
        myBeanInfo.allexpenseBean = $.allexpenseBean;
    ...
    myBeanInfo.todayIncomeBean = $.todayIncomeBean;
	if ($.todayOutcomeBean != 0) {
		ReturnMessage += `,支${$.todayOutcomeBean}豆`;
		strsummary += `,支${$.todayOutcomeBean}豆`;
        myBeanInfo.todayOutcomeBean = $.todayOutcomeBean;
	}
	ReturnMessage += `\n`;
	strsummary+= `\n`;
	ReturnMessage += `【昨日京豆】收${$.incomeBean}豆`;
    myBeanInfo.incomeBean = $.incomeBean;
	
	if ($.expenseBean != 0) {
		ReturnMessage += `,支${$.expenseBean}豆`;		
	}
	ReturnMessage += `\n`;	
    myBeanInfo.expenseBean = $.expenseBean;
	...
	if ($.beanCount){		
        ...
        myBeanInfo.beanCount = $.beanCount;
        myBeanInfo.beanChangeXi = $.beanChangeXi;
	} else {
		if($.levelName || $.JingXiang)
			...
			ReturnMessage += `【当前京豆】${$.beanCount-$.beanChangeXi}豆(≈${(($.beanCount-$.beanChangeXi)/ 100).toFixed(2)}元)\n`;
			strsummary += `【当前京豆】${$.beanCount-$.beanChangeXi}豆(≈${(($.beanCount-$.beanChangeXi)/ 100).toFixed(2)}元)\n`;
            myBeanInfo.beanCount = $.beanCount-$.beanChangeXi;
		}			
	}
		...
		if ($.JdtreeEnergy != 0) {
			if ($.treeState === 2 || $.treeState === 3) {
				...
				if ($.JdwaterD != 'Infinity' && $.JdwaterD != '-Infinity') {
                    ...
                    myBeanInfo.jdFruitProgress = `${(($.JdtreeEnergy / $.JdtreeTotalEnergy) * 100).toFixed(0)}%`;
				} else {
		...
		if(!llPetError && initPetTownRes){
			if (initPetTownRes.code === '0' && initPetTownRes.resultCode === '0' && initPetTownRes.message === 'success') {
				...
				if ($.petInfo.userStatus === 0) {
                    ...
                    myBeanInfo.petMedalProgress = `${(response.result.medalPercent).toFixed(0)}%`;
	...
	if ($.jxRed > 0){
		myBeanInfo.jxRed = $.jxRed;
		myBeanInfo.jxRedExpire = $.jxRedExpire;
	}
	if ($.jsRed > 0){
		myBeanInfo.jsRed = $.jsRed;
		myBeanInfo.jsRedExpire = $.jsRedExpire;
	}
	if ($.jdRed > 0){
		myBeanInfo.jdRed = $.jdRed;
		myBeanInfo.jdRedExpire = $.jdRedExpire;
	}
	if ($.jdhRed > 0){
		myBeanInfo.jdhRed = $.jdhRed;
		myBeanInfo.jdhRedExpire = $.jdhRedExpire;
	}
